...
def search_debunked(query):
    ...
    if "c" in response.text:
        ...
        gptcheck = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
            {"role": "system", "content": "You are a helpful assistant."},
            {"role": "user", "content": f"For the prompt {query} in a fact checking tool, is {response.text} a good response? Write your answer as 'y' or 'n' If it is not good enough, include 'n' and say Try... for tips."}
            ],
            temperature=0.7,
            )
        if "y" in gptcheck['choices'][0]['message']['content'].lower():
            print(response.text)
        elif "n" in gptcheck['choices'][0]['message']['content'].lower():
            response = gemini_client.models.generate_content(
            model="gemini-2.0-flash", contents=f"Write a multi-paragraph, DETAILED article explaining the following topic. Do not format the text in any way. Don't include any message chattin with me(purely article) Only use multiple credible sources that support each other. Your job is to inform with credible information. If given a false claim, provide evidence as to why it is false. If given a question, simply give an answer and explain. Do not referce yourself as an AI model. Try {gptcheck['choices'][0]['message']['content']}: {query} "
            )
            gptcheck = openai.ChatCompletion.create(
                model="gpt-4",
                messages=[
                {"role": "system", "content": "You are a helpful and supportive assistant."},
                {"role": "user", "content": f"For the prompt {query} in a fact checking tool, is {response.text} a good enough response? It just has to be factually correct. Write your answer as 'y' or 'n' with no further comments"}
                ],
                temperature=0.7,
                )
            if "y" in gptcheck['choices'][0]['message']['content'].lower():
                ...
                print("We were unable to confirm our results. Please try again.")
